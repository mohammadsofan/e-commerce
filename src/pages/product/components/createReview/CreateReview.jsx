...
export default function CreateReview({ productId, setRerender }) {
  ...
  const handleRating = (num) => {
    ...
    console.log(s);
    setStars(s);
    setReview((rev) => ({ ...rev, rating: num }));
  };

  const handleCommentChange = (e) => {
    const { name, value } = e.target;
    setReview((rev) => ({ ...rev, [name]: value }));
  };
  ...
  return (
    <div className="d-flex flex-column gap-1 align-items-start mt-4">
      <h4>Add Review</h4>
      {!stars.length ? (
        <div>
          <svg
